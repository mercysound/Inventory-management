// EmailBroadcastPanel.jsx — updated to include wholesale role
// Only the ROLES array and the emailBroadcastSchema reference changed.
// All other logic is identical to your original file.
...
const TEMPLATES = [
  ...
  { label: "Wholesale Deal", icon: "🏪", subject: "Exclusive Wholesale Offer — Limited Time",
    body: `<p>Dear Wholesale Partner,</p><p>We have a <strong>special bulk pricing offer</strong> available exclusively for our wholesale partners this week.</p><p>Contact us or visit the store to learn more.</p><p>Best regards,<br><strong>The Team</strong></p>` },
];

// ✅ Wholesale added here
const ROLES = [
...
export default function EmailBroadcastPanel({ users, roleColors }) {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (job.status === "done") {
          ...
          if (job.failed === 0) toast.success(`✅ All ${job.sent} emails delivered!`);
          else if (job.sent === 0) toast.error(`❌ Broadcast failed — 0 of ${job.total} delivered.`);
          else toast.warn(`⚠️ Partial: ${job.sent} sent, ${job.failed} failed.`);
        }
  ...
  // Extended roleColors to include wholesale
  const allRoleColors = {
    ...roleColors,
  ...
  return (
    <>
      <style>{`
        ...
        .ebp-rcpt-count{background:#0f172a;color:#fff;border-radius:20px;padding:2px 9px;font-size:11px;}
        ...
        .ebp-stat-val{font-size:20px;font-weight:800;line-height:1;}
        .ebp-stat-key{font-size:10px;opacity:.7;text-transform:uppercase;letter-spacing:.05em;margin-top:2px;}
      ...
      <div className="ebp-wrap">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="ebp-card">
          <div className="ebp-card-header">
            ...
            <button onClick={() => setShowTemplates(p => !p)} className="ebp-tpl-toggle">📋 Templates</button>
          </div>
          ...
          <div className="ebp-section" style={{ paddingTop: 14 }}>
            ...
            <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Important update from our team" className="ebp-input" maxLength={200} />
          ...
          <div className="ebp-section" style={{ paddingTop: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
              <label className="ebp-section-label" style={{ marginBottom: 0 }}>Message</label>
              <button onClick={() => setPreview(p => !p)} className="ebp-preview-toggle">{preview ? "✏️ Edit" : "👁 Preview"}</button>
            </div>
            {preview
              ? <div className="ebp-preview-box" dangerouslySetInnerHTML={{ __html: body || "<p style='color:#94a3b8'>Nothing to preview…</p>" }} />
              : <textarea value={body} onChange={e => setBody(e.target.value)} rows={9} placeholder={"<p>Dear Customer,</p>\n<p>Your message here…</p>"} className="ebp-textarea" />
            }
          </div>

          {/* Attachments */}
          <div className="ebp-section" style={{ paddingTop: 14, paddingBottom: 0 }}>
            <label className="ebp-section-label">Attachments (max {MAX_MB}MB each)</label>
          ...
          {jobResult && (() => {
            ...
            const progress = jobResult.total > 0 ? Math.round(((jobResult.sent + jobResult.failed) / jobResult.total) * 100) : 0;
            ...
            return (
              <div className={`ebp-result ${panelCls}`} role="status">
                <p className="ebp-result-title" style={{ color: titleColor }}>
                  ...
                  {allOk && "✅"} {partial && "⚠️"} {allFailed && "❌"}
                  {running ? `Sending… ${jobResult.sent + jobResult.failed} / ${jobResult.total}`
                    : allOk ? `All ${jobResult.sent} emails delivered`
                    : partial ? `Partial: ${jobResult.sent} sent, ${jobResult.failed} failed`
                    : `Failed — 0 of ${jobResult.total} sent`}
                </p>
                <div className="ebp-progress-track">
                  <div className="ebp-progress-fill" style={{ width: `${running ? Math.max(progress, 4) : 100}%`, background: barColor }} />
                </div>
                <div className="ebp-result-stats" style={{ color: titleColor }}>
                  <div className="ebp-stat-item"><span className="ebp-stat-val">{jobResult.total}</span><span className="ebp-stat-key">Total</span></div>
                  <div className="ebp-stat-item"><span className="ebp-stat-val" style={{ color: "#16a34a" }}>{jobResult.sent}</span><span className="ebp-stat-key">Delivered</span></div>
                  {jobResult.failed > 0 && <div className="ebp-stat-item"><span className="ebp-stat-val" style={{ color: "#dc2626" }}>{jobResult.failed}</span><span className="ebp-stat-key">Failed</span></div>}
                </div>
                {isDone && <button className="ebp-result-dismiss" style={{ color: titleColor }} onClick={() => setJobResult(null)}>Dismiss ✕</button>}
              </div>
            );
          })()}
          ...
          <div className="ebp-send-bar">
            ...
            <button onClick={handleSend} disabled={sending || recipientCount === 0} className="ebp-send-btn"
              ...
              {sending ? "Starting…" : jobResult?.status === "running" ? "Sending…" : "Send Email →"}
            </button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }} className="ebp-rcpt-card">
          ...
          <div className="ebp-rcpt-list">
              ...
              <div key={i} className="ebp-rcpt-row">
                ...
                {u.role && u.role !== "—" && (
                  <span className="ebp-rcpt-pill-role"
                    style={{ background: allRoleColors[u.role]?.bg || "#f1f5f9", color: allRoleColors[u.role]?.text || "#64748b", border: `1px solid ${allRoleColors[u.role]?.border || "#e2e8f0"}` }}>
                    {u.role}
                  </span>
                )}
              </div>
            ...
            {recipientCount === 0 && <div className="ebp-empty">📭 No recipients</div>}
          </div>
        </motion.div>
      </div>
    </>
