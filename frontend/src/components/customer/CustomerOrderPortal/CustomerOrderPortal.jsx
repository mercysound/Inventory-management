...
// buildInvoiceUrl is imported from src/utils/buildInvoiceUrl.js
// It builds an absolute URL so the iframe bypasses React Router.
...
const CustomerOrderPortal = () => {
  ...
  const handlePaymentSuccess = async (paystackResponse) => {
    ...
    try {
      ...
      // Build the final receipt URL using the shared helper.
      // buildInvoiceUrl produces an absolute URL so React Router is bypassed.
      const url = buildInvoiceUrl({
        customerName:  user?.name || "Customer",
        paymentMethod: "Paystack",
        mode:          "final",
