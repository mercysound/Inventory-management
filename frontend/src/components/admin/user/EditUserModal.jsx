...
export default function EditUserModal({ user, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) { toast.success("User updated"); onSuccess?.(); }
    } catch (err) {
    ...
    <>
      <style>{`
        ...
        .eum-modal{
          ...
          width:100%;
          max-width:520px;
          /* NOT using transform:translate(-50%,-50%) — that breaks on small screens */
          margin:auto;  /* vertically center when content shorter than viewport */
          flex-shrink:0;
          position:relative;
        }
        .eum-header{
      ...
      <motion.div
        className="eum-backdrop"
        initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
        onClick={e => { if (e.target === e.currentTarget) onClose(); }}
        role="dialog" aria-modal="true" aria-label={`Edit ${user?.name}`}
      >
        <motion.div
          className="eum-modal"
        ...
        >
          ...
          <div className="eum-header">
            ...
            <button onClick={onClose} className="eum-close" aria-label="Close modal">✕</button>
          </div>
            ...
            <div className="eum-body">
              <div className="eum-grid">
                <div className="eum-field">
                  ...
                  <input value={form.name} onChange={e=>set("name",e.target.value)} className={`eum-input${errors.name?" eum-input-err":""}`}/>
                  {errors.name&&<span className="eum-error">{errors.name}</span>}
                </div>
                <div className="eum-field">
                  <label className="eum-label">Email <span style={{color:"#ef4444"}}>*</span></label>
                  <input type="email" value={form.email} onChange={e=>set("email",e.target.value)} className={`eum-input${errors.email?" eum-input-err":""}`}/>
              ...
              <div className="eum-grid">
                <div className="eum-field">
                  ...
                  <input value={form.phone} onChange={e=>set("phone",e.target.value)} placeholder="+234 800 000 0000" className="eum-input"/>
                </div>
                <div className="eum-field">
                  <label className="eum-label">Role <span style={{color:"#ef4444"}}>*</span></label>
                  <select value={form.role} onChange={e=>set("role",e.target.value)} className={`eum-input${errors.role?" eum-input-err":""}`}>
              ...
              <div className="eum-field">
                ...
                <input value={form.address} onChange={e=>set("address",e.target.value)} placeholder="Street, city, state" className="eum-input"/>
              </div>
