        ...
        <div className="um-header">
          <div className="um-title-row">
            <div>
              ...
              <p className="um-subtitle">Manage team members, customers and communications</p>
            </div>
          </div>
        ...
        <div className="um-tabbar" role="tablist">
          {TABS.map(tab => (
            <button key={tab.id} role="tab" aria-selected={activeTab===tab.id}
              ...
              style={{ background:activeTab===tab.id?"#0f172a":"transparent", color:activeTab===tab.id?"#fff":"#64748b" }}>
              <span>{tab.icon}</span><span className="um-tab-lbl">{tab.label}</span>
            </button>
          ))}
        </div>

        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-5}} transition={{duration:0.18}}>
            {activeTab==="list" && (
              <UsersTable users={filtered} loading={loading}
                ...
                onDelete={handleDelete} onEdit={u=>setEditTarget(u)} roleColors={roleColors}
              />
            )}
