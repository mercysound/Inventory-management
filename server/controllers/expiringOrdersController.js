...
export const getExpiringOrders = async (req, res) => {
  try {
    ...
    if (!settings) settings = { orderExpiryHours: 48, reminderMode: "repeat", reminderIntervalHours: 6 };
    ...
    return sendResponse(res, 200, {
      orders: annotated,
      settings: {
        orderExpiryHours: settings.orderExpiryHours,
...
export const getExpiringOrdersCount = async (req, res) => {
  try {
    let settings = await SettingsModel.findOne().sort({ createdAt: 1 });
    if (!settings) settings = { orderExpiryHours: 48 };
